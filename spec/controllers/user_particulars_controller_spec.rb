...
RSpec.describe UserParticularsController, type: :controller do
    describe 'POST #confirm' do
        ...
        it 'renders new with an error if date of birth is in the future' do
            post :confirm, params: { date_of_birth: (Date.today + 1).to_s }
            expect(flash[:error]).to include('Date of birth cannot be in the future.')
            expect(response).to render_template(:new)
        end

        it 'renders new with an error if date of arrival is in the future' do
        ...
        it 'renders new with an error if date of arrival is earlier than date of birth' do
